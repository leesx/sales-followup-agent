...
const salesStages = ["初步接触", "需求确认", "方案评估", "商务谈判", "决策审批", "成交", "丢单"];
...
export function analyzeCustomer(customer) {
  ...
  const stageSuggestion = buildStageSuggestion(customer, latestFollowup);
...
function buildStageSuggestion(customer, latestFollowup) {
  const signalText = [latestFollowup?.content, ...customer.signals].filter(Boolean).join("，");
  const currentStage = customer.stage;

  if (currentStage === "初步接触" && hasAny(signalText, ["需求", "痛点", "样本", "演示"])) {
    return makeStageSuggestion(customer, "需求确认", 0.68, "客户已经表达明确需求或愿意提供样本，可以推进到需求确认。");
  }

  if (currentStage === "需求确认" && hasAny(signalText, ["方案", "演示", "报价", "预算"])) {
    return makeStageSuggestion(customer, "方案评估", 0.72, "客户开始关注方案、预算或演示内容，可以推进到方案评估。");
  }

  if (currentStage === "方案评估" && hasAny(signalText, ["预算已通过", "采购要求", "合同", "条款", "ROI"])) {
    return makeStageSuggestion(customer, "商务谈判", 0.82, "预算或采购材料已经进入讨论，商机可以从方案评估推进到商务谈判。");
  }

  if (currentStage === "商务谈判" && hasAny(signalText, ["CEO", "采购发起", "供应商准入", "审批", "合同条款"])) {
    return makeStageSuggestion(customer, "决策审批", 0.78, "关键决策或采购审批流程已经启动，可以推进到决策审批。");
  }

  if (currentStage === "决策审批" && hasAny(signalText, ["合同已签", "打款", "成交", "已付款"])) {
    return makeStageSuggestion(customer, "成交", 0.9, "合同或付款信号已经确认，可以标记为成交。");
  }

  return null;
}

function makeStageSuggestion(customer, suggestedStage, confidence, reason) {
  return {
    customerId: customer.id,
